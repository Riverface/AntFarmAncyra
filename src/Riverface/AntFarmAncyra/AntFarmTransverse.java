...
public class AntFarmTransverse extends BaseDurationAbility {
            ...
            if (well != null && fleet.isInHyperspace() && canUseToJumpToSystem()) {
                ...
                if (crCostFleetMult > 0) {
                    for (FleetMemberAPI member : getNonReadyShips()) {
                        if ((float) Math.random() < com.fs.starfarer.api.impl.campaign.abilities.EmergencyBurnAbility.ACTIVATION_DAMAGE_PROB) {
